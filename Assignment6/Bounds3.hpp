                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    //但当 dir.x 为负时，计算得到的 t 与几何上的“near-face / far-face”顺序会反过来（因为当 dir.x < 0，越小的 x 对应越大的 t），
    //因此需要根据方向符号决定哪一个是进入（t_near）哪一个是离开（t_far）。
    if (!dirIsNeg[0]) std::swap(t_x_min, t_x_max);
    if (!dirIsNeg[1]) std::swap(t_y_min, t_y_max);
    if (!dirIsNeg[2]) std::swap(t_z_min, t_z_max);

    float t_in = std::max({t_x_min, t_y_min, t_z_min});
    float t_out = std::min({t_x_max, t_y_max, t_z_max});
